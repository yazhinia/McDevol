...
class BYOLmodel(nn.Module):
    ...
    def forward(self, x, xt):
        ...
        # if flag:
        #     # marker gene loss
        #     # key = random.choice(list(self.marker.keys()))
        #     for key in self.marker.keys():
        #         print(key, 'key')
        #         cindx = self.marker[key]
        #         if self.usecuda:
        #             cindx = cindx.cuda()
        #         marker_indices = torch.isin(cindices, cindx)
        #         if marker_indices.sum() > 1:
        #             marker_loss = self.cosine_sim(\
        #                 z1_online[marker_indices], z1_target[marker_indices])
        #             byol_loss += 1e-2 * marker_loss
    ...
    def cosine_sim(self, z1_online, z1_target):
        """ cosine similarity between contigs with the same SCMGs """
        cosine_sim_matrix = F.cosine_similarity(z1_online.unsqueeze(1),\
            z1_target.unsqueeze(0), dim=-1)
        mask = torch.eye(z1_online.shape[0], device=cosine_sim_matrix.device).bool()
        cosine_sim_matrix.masked_fill_(mask, 0)
        num_off_diag_elements = mask.numel() - mask.sum().item()
        cosine_sim_loss = cosine_sim_matrix.sum() / num_off_diag_elements
    ...
    def process_batches_withpairs(self,
        ...
        *args):
        ...
        for in_data in dataloader:
            ...
            print(pairs, pairindices_1, pairindices_2)
            if self.usecuda:
