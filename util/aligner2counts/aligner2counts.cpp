...
const unsigned int FLAG_2 = 2;      // 0x2 each segment properly aligned according to the aligner (paired)
const unsigned int FLAG_3 = 4;      // 0x4 segment unmapped
const unsigned int FLAG_4 = 8;      // 0x8 next segment in the template unmapped xxx
...
const unsigned int FLAG_7 = 64;     // 0x40 the first segment in the template (forward read R1)
const unsigned int FLAG_8 = 128;    // 0x80 the last segment in the template (reverse read R2)
...
void counting(AlnParser &parsedaln, ContigsMap &contigs_map, ContigLinks &contig_links, float &sequenceidentity) {
    ...
    if (it->sequence_identity >= sequenceidentity) {
        ...
        if (parsedaln.size() > 0) {
            ...
            float value_unpaired = frac_contigs_mapped / 2.0f;
            float value_added_to_paired = value_unpaired * non_paired_count;
            float value_added_per_paired = value_added_to_paired / paired_count;
            // std::cout << paired_count << " paired count " << non_paired_count << " non paired " << frac_contigs_mapped << " frac contigs " << value_added_per_paired << " value added \n";
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 1 || strcmp(argv[1], "--help") == 0 || strcmp(argv[1], "-h") == 0) {
        std::cout << "aligner command | ./bam2counts working_dir outputname" << "\n";
        return 1;
    }
    ...
    while(std::getline(std::cin, line)) {
        if (line.size() > 0) {
            if (line[0] == '@') {
                ...
                construct_map(contigs_map, line, minlength);
            } else {
                std::istringstream iss(line);
                iss >> currentread_id;
                ...
                // std::cout << currentread_id << " " << contig_id << " read and contigs\n";
                ...
                // std::cout << currentread_id << " " << contig_id << "\n";
    ...
    // while end
    counting(parsedaln, contigs_map, contiglinks, sequenceidentity); // process last read alignment
    parsedaln.clear();

    std::ofstream outfile, linkfile;
    outfile.open(tmp_dir + '/' + outname + "_count");
    linkfile.open(tmp_dir + '/' + outname + "_countlinks");

    for (auto const & k: contigs_map) {
        outfile << k.first << " " << k.second << "\n";
    }

    for (auto const & k: contiglinks) {
        linkfile << k.first.first << " " << k.first.second << " " << k.second << "\n";
    }
